...
#' "resources_OP", "top_ranks_OP", "top_ranks_overlap","runtime", and
...
#' and dilution stage.
#'
#' "top_ranks_overlap": How the diluted top predictions related to the
#' undiluted predictions in terms of jaccard index.
#'
...
# 1. Preparing resource_Dilute() Inputs ----------------------------------------
{
  ...
  # Apply get_top_ranks for each method's results on OP_0 (i.e. undiluted)
  top_ranks_OP_0 <- list()
...
# 3. Rerun Liana and contrast predictions --------------------------------------
{
  ...
  top_ranks_OP <- map(methods_list, function (method) {
    # Per method, map over every dilution getting the top ranks each time
    map(liana_results_OP[[method]], function(results) {
      get_top_ranks(data_set = results,
                    method = method,
                    top_n = number_ranks[[method]])
    })
  }) %>% # Format everythign to have LR_IDs and LR_Pairs
    map_depth(., .depth = 2, format_top_ranks)
