...
# reformat_Results()
{
  ...
  reformat_Results <- function(results) {
    ...
    # This is the segment of the results containing runtim, testdata and
    # top_ranks_overlap, none of which need to formatted further
    segment_formatted <-
    ...
    # This is the segment of the results containing resources
    segment_resources <-
      results[names(results) %in% intersect(names(results),
                                            c("resources_OP"))]
    ...
    # Format resources, we transpose at a deeper level
    segment_resources <- segment_resources %>%
...
# extract_top_ranks()
{
  ...
  extract_top_ranks <- function(results) {
    ...
    # get just the top_ranks analysis information and bind rows
    collated_top_ranks_overlap <- results$top_ranks_overlap %>%
      bind_rows() %>%
