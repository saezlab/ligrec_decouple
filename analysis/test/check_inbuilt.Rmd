...
# 984 interactions for Original CellChatDB, 2,551 for OmniPath
cc_omnipath %>% glimpse()
cc_inbuilt %>% glimpse()
```
...
```{r natmi}
natmi_omni <- read.csv("~/Repos/ligrec_decouple/data/input/connectomeDB2020.csv") %>%
    unite(Ligand.gene.symbol, Receptor.gene.symbol,
          col = "Pair.Name", remove = FALSE)

natmi_inbuilt <- read.csv("~/Repos/ligrec_decouple/data/input/lrc2p.csv") %>%
    unite(Ligand.gene.symbol, Receptor.gene.symbol,
          col = "Pair.Name", remove = FALSE)
