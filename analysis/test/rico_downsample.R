...
# Try the mean
print(Matrix::mean(logcounts(seurat_conv)))


# Generate single-cell experiment-like sparse mat
mat <- qlcMatrix::rSparseMatrix(nrow=20000,
                                ncol = 1e7,
                                nnz = 1e8)
...
# calculate mean
x1 <- system.time(r_mean <- Matrix::mean(mat))
# fast mean
x2 <- system.time(pog_mean <- fast_mean(mat))

identical(r_mean, pog_mean)
...
sum(xd@x)/(as.numeric(nrow(xd)) * as.numeric(ncol(xd)))
system.time(Matrix::mean(xd))

difftime(system.time(Matrix::mean(xd)),
         system.time(sum(xd@x)/(as.numeric(nrow(xd)) * as.numeric(ncol(xd)))))
